...
到 [Releases](https://github.com/liuem/LCEDA-bom-search-app/releases) 下载最新 `*.apk`，
手机上点开安装（需允许「安装未知应用」权限）。

- 若之前装过开发调试版（签名不同），先卸载旧版再装；
- Release 包使用 30 年有效期证书签名，后续更新包可直接覆盖安装。
...
2. 导入 BOM，两种方式任选：
   - **浏览器上传（推荐）**：手机 App 打开后，电脑浏览器访问手机主页顶部显示的地址
     （`http://手机IP:8266/`），页面上直接选 BOM 文件上传；
   - App 内「导入 BOM 文件」按钮走系统文件选择器；
3. 设置套数（多板拼贴时按倍数累计用量）；
